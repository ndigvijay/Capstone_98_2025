...
def infer_malware_type(malware_names):
    ...
    for name in malware_names:
        ...
        if 'VIRUS' in name_upper:
            malware_types.add('Virus')
...
def load_embeddings_and_features_from_mongodb(cache):
    ...
    cursor = collection.find({"graph_embedding": {"$exists": True}}, no_cursor_timeout=True)
        ...
        if embedding:
            ...
            if request_count >= 4:
                ...
                if elapsed_time < 60:
                    ...
                    logger.info(f"Sleeping for {sleep_time} seconds to comply with rate limit.")
